...
class ChemicalInfoApp(QWidget):
    ...
    def fetch_file_data(self):
        ...
        for label_text in ["Soubor", "Vzorec", "CID", "Schéma"]:
            label = QLabel(label_text)
            self.layout().addWidget(label)

        # Update the labels in the second row with the fetched data
        self.result_labels["iupac"].setText(file_path)
    ...
    def fetch_pubchem_data(self):
        ...
        if response.status_code == 200:
            ...
            self.result_labels["iupac"].setText(iupac_name)
            self.result_labels["trivial"].setText(trivial_name)
            self.result_labels["cid"].setText(str(cid))
    ...
    def fetch_pdb_data_from_api(self, pdb_code):
        ...
        try:
            ...
            # Add labels for the columns in the first row
            for label_text, result_label_key in zip(["Název", "Molekulová hmotnost", "PDB", "Schéma"],
                                                   ["iupac", "trivial", "cid"]):
                label = QLabel(label_text)
                self.layout().addWidget(label)

                # Update the labels with the fetched data
                self.result_labels[result_label_key].setText(str(protein_name) if result_label_key == "iupac"
                                                              else f"{molecular_mass} Da" if result_label_key == "trivial"
                                                              else str(pdb_code))
